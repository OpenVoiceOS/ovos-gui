...
class TestVersion(unittest.TestCase):
    ...
    def test_current_version(self):
        """Test current version values match expected values."""
        from ovos_gui.version import VERSION_MAJOR, VERSION_MINOR, VERSION_BUILD, VERSION_ALPHA
        # Current known version from reading the file
        self.assertEqual(VERSION_MAJOR, 1)
        self.assertEqual(VERSION_MINOR, 3)
        self.assertEqual(VERSION_BUILD, 5)
        self.assertEqual(VERSION_ALPHA, 3)

    def test_current_version_string(self):
        """Test current __version__ string."""
        from ovos_gui.version import __version__
        # Should be 1.3.5a3 based on current constants
        self.assertEqual(__version__, "1.3.5a3")
